  ...
  try {
    ...
    if (!response.ok) {
      res
        ...
        .json({ images: [], error: "Failed to fetch source page" });
      return;
    }
    ...
    const unique = Array.from(new Set(matches)).filter((u) =>
      /\b(orig|1200x|800x)\b/.test(u),
    );

    const images = (
      unique.length ? unique : Array.from(new Set(matches))
    ).slice(0, 8);
